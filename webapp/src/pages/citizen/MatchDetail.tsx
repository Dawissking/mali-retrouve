...
import { useAgentMatchDetail, useAcceptMatch, useRejectMatch } from "../../hooks/useMatches";
...
export default function MatchDetail() {
  ...
  const acceptMatch = useAcceptMatch(id!);
  const rejectMatch = useRejectMatch(id!);
      ...
      <div className="card p-8">
        ...
        {(match.status === "CANDIDATE" || match.status === "POTENTIAL" || match.status === "TO_VERIFY") && (
          <div className="mt-8 pt-6 border-t border-gray-200 flex gap-4">
            <Button
              variant="primary"
              onClick={() => acceptMatch.mutate(undefined, { onSuccess: () => {} })}
            ...
            <Button
              variant="danger"
              onClick={() => rejectMatch.mutate(undefined, { onSuccess: () => {} })}
              isLoading={rejectMatch.isPending}
            >
              Refuser
            </Button>
          </div>
